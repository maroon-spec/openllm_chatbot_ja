...
# MAGIC
# MAGIC
# MAGIC 我々のアプリケーションでは、**Databricks HPの日本語ブログ**や**FAQ**、**データブリック社員がポストしたQitta記事**から一連のドキュメントを抽出しました。これらのデータセットはHuggingFaceに公開されており、サンプルとしてご利用いただけます。これらの3つのドキュメントソースを用いることで、我々のbotは深い専門性を持って、この領域において適切な質問に反応することができます。
# MAGIC
# MAGIC </p>
# MAGIC
# MAGIC <img src='https://sajpstorage.blob.core.windows.net/maruyama/webinar/llm/dataflow1.png' width=900>
# MAGIC
# MAGIC </p>
# MAGIC
...
# MAGIC %md ##Step 1: サンプルデータをテーブルにロード
# MAGIC
# MAGIC 日本語Q&AデータセットはHugging Face Hubから[アクセス](https://huggingface.co/datasets/yulanfmy/databricks-qa-ja)できます。これをSparkデータフレームに変換します。
...
# MAGIC
# MAGIC * response - ユーザーの質問に適した情報を含む可能性のあるドキュメントのテキストやナレッジベースの回答
...
long_text = (
  raw_inputs
    .select('response') # responseフィールドのみを取得
    .orderBy(fn.expr("len(response)"), ascending=False) # 長さでソート
    .limit(1) # トップ1を取得
     .collect()[0]['response'] # テキストを変数に格納
  )
...
# MAGIC %md ## 入力をチャンクに分割
# MAGIC
# MAGIC 今回は元のデータセットがチャンク分割されているのか、Max chunk sizeを超えるものがありませんでした。そのためデータ量は同じになりました。
...
chunked_inputs = (
  raw_inputs
    .withColumn('chunks', get_chunks('response')) # テキストをチャンクに分割
    .drop('response')
    .withColumn('num_chunks', fn.expr("size(chunks)"))
    .withColumn('chunk', fn.expr("explode(chunks)"))
    .drop('chunks')
    .withColumnRenamed('chunk','response')
  )
...
# 検索可能なテキスト要素を抽出
text_inputs = inputs['response'].to_list()

# メタデータの抽出
metadata_inputs = (
  inputs
    .drop(['response','num_chunks'], axis=1)
